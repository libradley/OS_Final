...
do_variable_assignment(struct command const *cmd, int export_all)
{
  for (size_t i = 0; i < cmd->assignment_count; ++i) {
    ...
    /* TODO Assign */
    if (vars_set(a->name, a->value) != 0) return -1; // assignment failed
    /* TODO Export (if export_all != 0) */
    if (export_all && vars_export(a->name) != 0) return -1; // export failed
  }
  return 0;
}
  ...
  for (size_t i = 0; i < cmd->io_redir_count; ++i) {
    ...
    file_open:;
      ...
      if (move_fd(fd, r->io_number) < 0) {
        perror("Error: Failed to move file descriptor");
        goto err;
      }
    }
    ...
    err: /* TODO Anything that can fail should jump here. No silent errors!!! */
      fprintf(stderr, "Error: I/O redirection failed\n");
      status = -1;
    }
  }
  return status;
...
run_command_list(struct command_list *cl)
{
  ...
  for (size_t i = 0; i < cl->command_count; ++i) {
    ...
    downstream_pipefd = pipe_fds[STDOUT_FILENO]; // was int const
    // int has_downstream_pipe = (downstream_pipefd >= 0); // was int const
     ...
     */
    int did_fork = 0; /* TODO (will fork if command is not built-in or if built-in command should run in background)*/

    if (!is_builtin || !is_fg) {
    if (is_builtin && strcmp(cmd->words[0], "exit") == 0) {
        // Execute exit builtin directly in the parent process
        builtin(cmd, NULL);
        // If we reach here, something went wrong with the exit
        fprintf(stderr, "bigshell ERROR: exit failed\n");
        goto err;
    }
    ...
    if (child_pid == 0) {
      if (is_builtin) {
        ...
        fprintf(stderr, "BigShell Error: %s command not found\n", cmd->words[0]);
        exit(127);
        // err(127, 0); /* Exec failure -- why might this happen? Command DNE*/
