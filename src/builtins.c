...
builtin_cd(struct command *cmd, struct builtin_redir const *redir_list)
{
   ...
   */
    ...
    target_dir = cmd->words[1];     //basically, no arguments = home directory, argument? change to that directory
    }
  if (chdir(target_dir) != 0) {     //if moving to target directory fails - returns non zero return
    dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "cd ERROR: %s: %s\n", target_dir, strerror(errno)); //print the error cd target_dir and error
    return -1;
  }
  char cwd[PATH_MAX];
  if (getcwd(cwd, sizeof(cwd)) != NULL) {                 //change was succesful, time to update the PWD variable
    if (vars_set("PWD", cwd) < 0) {
      dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "cd ERROR: failed to update PWD\n");
      return -1;
    }
  } else {
    dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "cd ERROR: failed to get current directory\n");   //this way we handle both stages of potential errors
    return -1;
  }
...
builtin_exit(struct command *cmd, struct builtin_redir const *redir_list)
{
  /* TODO: Set params.status to the appropriate value before exiting */
   int exit_status = params.status;
    if (cmd->word_count > 1) {
        char *end;
        long status = strtol(cmd->words[1], &end, 10);
        if (*end != '\0' || status < 0 || status > 255) {
            dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "exit: Illegal number: %s\n", cmd->words[1]);
            return 1;
        }
