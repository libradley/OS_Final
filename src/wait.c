...
wait_on_fg_pgid(pid_t const pgid)
{
  ...
  if (kill(-pgid, SIGCONT) < 0) {
        perror("Error: Failed to send SIGCONT to process group");
        return -1;
    }
  int wstatus = 0;
 ...
 if (isatty(STDIN_FILENO)) {
  ...
  for (;;) {
     ...
     */
      ...
      params.status = WSTOPSIG(status) + 128;
      break;
    }
      ...
      if (waitpid(-pgid, NULL, WNOHANG) == 0) {
        // There are still processes in the group
        continue;
      }
      jobs_remove_pgid(pgid);
      break;
  ...
  if (is_interactive) {
    ...
    if (isatty(STDIN_FILENO)) {
      ...
      if (tcsetpgrp(STDIN_FILENO, shell_pgid) < 0) {
        if (errno != ENOTTY) {
          perror("Error: Failed to restore shell as foreground process group");
          goto err;
        }
      }
    }
